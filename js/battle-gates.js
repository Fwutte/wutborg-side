(() => {
  ...
  class BattleGame {
    constructor() {
      this.canvas=document.getElementById("battle-canvas");this.renderer=new ArenaRenderer(this.canvas);this.audio=new AudioManager();this.progress=this.loadProgress();this.selectedLevel=Math.min(this.progress.unlockedLevel,LEVELS.length)-1;this.run=new BattleRun(LEVELS[this.selectedLevel],this.progress.upgrades);this.lastFrame=performance.now();this.savedResult=false;this.paused=false;
      ...
      this.bind();this.populateLevels();this.updateUpgradeUI();this.refresh();requestAnimationFrame(time=>this.loop(time));
    }
    ...
    selectLevel(index){if(LEVELS[index].id>this.progress.unlockedLevel)return;this.selectedLevel=index;this.run.load(LEVELS[index],this.progress.upgrades);this.populateLevels();this.refresh();}
    ...
    start(){this.run.load(LEVELS[this.selectedLevel],this.progress.upgrades);this.run.start();this.savedResult=false;this.paused=false;this.els.menu.hidden=true;this.els.result.hidden=true;this.els.pause.hidden=true;this.audio.tone(392,.13,"square",.035);this.setMessage("Vælg en port – byg en kombokæde med gode valg.");this.refresh();}
    showMenu(){this.run.load(LEVELS[this.selectedLevel],this.progress.upgrades);this.paused=false;this.els.menu.hidden=false;this.els.result.hidden=true;this.els.pause.hidden=true;this.populateLevels();this.updateUpgradeUI();this.refresh();}
